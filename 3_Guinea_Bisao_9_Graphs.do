...
use "${d_raw}\working\GNB_individual_reg_Full.dta" , clear 

	foreach var in inc_by_hour inc_d_total inc_d_selfw inc_d_salw inc_by_hour_IMP {
		* windsorization 
		sum `var'_usd , d
		replace `var'_usd = r(p1) if `var'_usd<r(p1)     & `var'_usd != . 
		replace `var'_usd = r(p95) if `var'_usd > r(p95) & `var'_usd != .
		sum `var'_usd , d
	}
...
gen male = female == 0
gen sector2_6 = 1 if sector2_ == . & productivity != . 
...
label define sex_fem 100 "Female" , modify 
	...
	desc urban w_wap female born_here educ_1 educ_2 educ_3 educ_4 informality inc_by_hour_IM inc_by_hour_us inc_d_total inc_d_selfw inc_d_salw week_hours neet
...
foreach categ in urban female born_here {
	...
	* local categ = "urban"
		...
		* Merge Categories
		...
		export excel using "${dir_out}/Tables/Z_Descriptive_statistics.xlsx" if type != "obs" , sheet("GNB By `byvar'" , replace)  firstrow(varl)	
		export excel using "${dir_out}/Tables/Z_Descriptive_statistics_obs.xlsx" if type == "obs" , sheet("GNB By `byvar'" , replace)  firstrow(varl)	
	restore 
		...
		* Export the total
		preserve
			...
			export excel using "${dir_out}/Tables/Z_Descriptive_statistics.xlsx" if type != "obs"  , sheet("GNB Total" , replace)  firstrow(varl)
			
			export excel using "${dir_out}/Tables/Z_Descriptive_statistics_obs.xlsx" if type == "obs"  , sheet("GNB Total" , replace)  firstrow(varl)	
...
** Hours worked CDF
	...
	* xtitle(Hours) xtitle("# Hours worked" )
	 ...
	 graph export "${dir_out}/Graphs/GNB 1. Hours worked by sex.png",  as(png)    replace width(1995)  height(1452)	
...
replace inc_by_hour = . if inc_by_hour < 0
	...
	*
	foreach inc in imputed_inc_h_usd inc_by_hour_usd inc_d_total inc_d_selfw inc_d_salw {
		...
		sum `inc'  [ aweight = wgt ] if female == 1 
			...
			qui local mu_f = round(`mu_f'  ,  0.04)
			qui local vr_f = round(`r(sd)',  0.01)

		sum `inc'  [ aweight = wgt ] if female == 0 
			qui local mu_m = round(2*`r(mean)',  0.001)
			qui local mu_m = round(`mu_m'*(1/2) +.0001 , 0.05)
	...
	note("Females: {&mu}{subscript:f}= `mu_f' {&sigma}{subscript:f}= `vr_f'" "Males:     {&mu}{subscript:m}= `mu_m' {&sigma}{subscript:m}= `vr_m'" , size (medsmall) position(4) ring(0) margin(medlarge))  xtitle("" ) ylabel( 0(20)100, labsize(small)) $inner_grid
		...
		* xtitle(Hours)  xtitle("`vrlab' in logs" )
		graph export "${dir_out}/Graphs/GNB 1. `vrlab' by sex.png",  as(png)    replace width(1995)  height(1452)
	...
	kdensity ln_`inc'  [ aweight = wgt ] if female == 0 `limit' , nograph generate(fx_m) at(x) `bw'
		...
		* xtitle("`vrlab' in logs", size(small))
		...
		graph export "${dir_out}/Graphs/GNB 2. Kernel `vrlab' by sex.png",  as(png)    replace width(1995)  height(1452)
	}
	...
	** Pies Main Activity
	...
	graph export "${dir_out}/Graphs/GNB 3. Main activity female.png",   replace width(1995)  height(1452)
	...
	graph export "${dir_out}/Graphs/GNB 3. Main activity Male.png",   replace width(1995)  height(1452)
	...
	* Employment composition 
	...
	graph export "${dir_out}/Graphs/GNB 4. Employment composition.png",   replace width(1995)  height(1452)
	...
	* Sectorial composition 
	...
	graph export "${dir_out}/Graphs/GNB 5. Sectorial composition.png",   replace width(1995)  height(1452)
	...
	* Kernel density "ln_inc_by_hour_usd"  By type of contract
	 ...
	 graph export "${dir_out}/Graphs/GNB 2. Kernel by contract.png",  as(png)    replace width(1995)  height(1452)
	 ...
	 * Kernel density "ln_inc_by_hour_usd" by SECTOR 
	 ...
	 $noaxis_tit ylabel(, noticks nolabels)
	  graph export "${dir_out}/Graphs/GNB 2. Kernel by sector.png",  as(png)    replace width(1995)  height(1452)
	 ...
	 graph export "${dir_out}/Graphs/GNB 2. Kernel by sector male.png",  as(png)    replace width(1995)  height(1452)
	 ...
	 * legend(lab(1 "Agriculture") lab(2 "Manufacture") lab(3 "Trade and distribution services") lab(4 "Low-skilled services") lab(5 "High-skilled services")  pos(6) col(2))
	 graph export "${dir_out}/Graphs/GNB 2. Kernel by sector female.png",  as(png)    replace width(1995)  height(1452)
	 ...
	 graph export "${dir_out}/Graphs/GNB 6. Share sector by sex.png",  as(png)    replace width(1995)  height(1452)
	 
	 graph hbar (sum) female (sum ) male if inc_by_hour_usd != . , over(empstat_ , ) legend(order(1 "Female" 2 "Male"))  ylabel(, nogrid) name(empstat_sex , replace)
	 graph export "${dir_out}/Graphs/GNB 6. Share emp_categ by sex.png",  as(png)    replace width(1995)  height(1452)
	 
	 graph hbar (sum) female (sum ) male if inc_by_hour_usd != . , over(edu_categ , ) legend(order(1 "Female" 2 "Male"))  ylabel(, nogrid) name(educ_sex , replace)
	 graph export "${dir_out}/Graphs/GNB 6. Share educ by sex.png",  as(png)    replace width(1995)  height(1452)
...
*------------------------------------------------------------------------------*
	...
	*
	rename agesq age_sq
	cap noi mkdir "${dir_out}/Tables/GNB"
	foreach inc in ln_inc_by_hour_usd ln_inc_d_total ln_inc_d_selfw ln_inc_d_salw  {
		...
		esttab OB_1 OB_2 OB_3 using "${dir_out}/Tables/GNB/1. OB Decomposition in `vrlab'.csv" ,  stats(N ) label replace addnotes("Group 1 == Males. Group 2 == Females" )  title("Oaxaca Blinder Decomposition") b(3) t(3)
	...
	* inc_d_salw
	foreach inc in inc_by_hour inc_d_total inc_d_selfw inc_d_salw  {  
		...
		*	
		...
		nopomatch age age_sq  $edulvl	$hh_charac	`add_control'	, outcome(`inc') by(male) fact(wgt) sd filename("${dir_out}/Tables/GNB/OB_N_1_`inc'") replace
		*
		restore
		
		preserve
		cap noi drop _supp _match
		nopomatch age age_sq  $edulvl	$sector		$hh_charac   `add_control'  , outcome(`inc') by(male) fact(wgt) sd filename("${dir_out}/Tables/GNB/OB_N_2_`inc'") replace
		*
		restore
		
		preserve
		cap noi drop _supp _match
		nopomatch age age_sq  $edulvl	$sector		$hh_charac	`add_control'  , outcome(`inc') by(male) fact(wgt) sd filename("${dir_out}/Tables/GNB/OB_N_3_`inc'") replace
		restore
	}
	...
	preserve
		...
		* ln_inc_by_hour
		foreach inc in  inc_by_hour inc_d_total inc_d_selfw inc_d_salw  {
			append using "${dir_out}/Tables/GNB/OB_N_1_`inc'"
			append using "${dir_out}/Tables/GNB/OB_N_2_`inc'"
			append using "${dir_out}/Tables/GNB/OB_N_3_`inc'"
			replace inc_type = "`inc'" if inc_type == ""
			
			erase "${dir_out}/Tables/GNB/OB_N_1_`inc'.dta"
			erase "${dir_out}/Tables/GNB/OB_N_2_`inc'.dta"
			erase "${dir_out}/Tables/GNB/OB_N_3_`inc'.dta"
			
		}
		export excel using "${dir_out}/Tables/Z_OB_Decomposition_Nopo_summary.xlsx" ,  sheet("GNB By sex" , replace)  firstrow(varl)
	restore 
	 ...
	 /*
	 ***** FIRMS *****
	 use "${d_raw}\working\GNB_na_firm2021.dta"
	 *-------------------------------------------------------------------------------
*-------------------------Generating graphs------------------------------
*-------------------------------------------------------------------------------
...
*----------
tab size_enterprise, gen(size_enterprise_)
graph pie size_enterprise_1 size_enterprise_2 size_enterprise_3 size_enterprise_4, ///
	plabel(_all percent) ///
	legend(lab(1 "1 worker") lab(2 "2 workers") lab(3 "3 workers") lab(4 "More than 3 workers") pos(6) col(2))
	
* graph export "${root1}Outputs\Figures\GNB\GNB_pie_composition_enterprises_size.png", replace

*Excel
preserve
collapse (sum) size_enterprise_1 size_enterprise_2 size_enterprise_3 size_enterprise_4
export excel using "${root1}Outputs\Tables\GNB\GNB_composition_enterprises_size.xlsx", firstrow(variables) replace
restore
*----------

*----------
graph hbar (mean) total_fam_workers total_paid_workers, ///
    over(size_enterprise, label(labsize(large))) ///
    stack percentages ///
    legend(lab(1 "Family workers") lab(2 "Paid workers") pos(6) col(2) size(large)) ///
    ytitle("Percentage", size(large)) 
* graph export "${root1}Outputs\Figures\GNB\GNB_bar_composition_enterprises_size_worker.png", replace
*------------

*----------
tab female, gen(female_)

graph hbar (mean) female_1 female_2, ///
    over(size_enterprise, label(labsize(large))) ///
    stack percentages ///
    legend(lab(1 "Male") lab(2 "Female") pos(6) col(2) size(large)) ///
    ytitle("Percentage", size(large)) 
* graph export "${root1}Outputs\Figures\GNB\GNB_bar_composition_enterprises_size_gender.png", replace
*------------

*----------
graph hbar (mean) educ_1 educ_2 educ_3 educ_4, ///
    over(size_enterprise, label(labsize(large))) ///
    stack percentages ///
    legend(lab(1 "Less than basic") lab(2 "Basic") lab(3 "Intermediate") lab(4 "Advanced") pos(6) col(2) size(large)) ///
    ytitle("Percentage", size(large)) 
*graph export "${root1}Outputs\Figures\GNB\GNB_bar_composition_enterprises_size_educ.png", replace
*------------

*----------
graph hbar (mean)  empstat_1 empstat_2 empstat_3, ///
    over(size_enterprise, label(labsize(large))) ///
    stack percentages ///
    legend(lab(1 "Self-employee/own boss") lab(2 "Salaried workers") lab(3 "Other workers" ) pos(6) col(2) size(large)) ///
    ytitle("Percentage", size(large)) 
* graph export "${root1}Outputs\Figures\GNB\GNB_bar_composition_enterprises_size_empstat.png", replace
*------------

*----------
graph hbar (mean) sector_1 sector_2 sector_3 , ///
    over(size_enterprise, label(labsize(large))) ///
    stack percentages ///
    legend(lab(1 "Agriculture") lab(2 "Manufacture") lab(3 "Services"  ) pos(6) col(2) size(large)) ///
    ytitle("Percentage", size(large)) 
* graph export "${root1}Outputs\Figures\GNB\GNB_bar_composition_enterprises_size_sector.png", replace
*------------


*------------
twoway kdensity productivity_b if size_enterprise == 1 || kdensity productivity_b if size_enterprise == 2 || kdensity productivity_b if size_enterprise == 3 || kdensity productivity_b if size_enterprise == 4 , legend(lab (1 "1 worker") lab(2 "2 workers") lab(3 "3 workers") lab(4 "More than 3 workers")  pos(6) col(2)) xsize(3) ysize(3)  xtitle("Productivity")
* graph export "${root1}Outputs\Figures\GNB\GNB_kden_productivity_enterprises_size.png", replace
*------------

*------------
twoway kdensity productivity_b if size_enterprise == 1 || kdensity productivity_b if size_enterprise == 2 || kdensity productivity_b if size_enterprise == 3 || kdensity productivity_b if size_enterprise == 4 , legend(lab (1 "1 worker") lab(2 "2 workers") lab(3 "3 workers") lab(4 "More than 3 workers")  pos(5) col(2))   xtitle("Productivity") by(female)
* graph export "${root1}Outputs\Figures\GNB\GNB_kden_productivity_enterprises_size_gender.png", replace
*------------

*------------
twoway kdensity productivity_b if size_enterprise == 1 || kdensity productivity_b if size_enterprise == 2 || kdensity productivity_b if size_enterprise == 3 || kdensity productivity_b if size_enterprise == 4 , legend(lab (1 "1 worker") lab(2 "2 workers") lab(3 "3 workers") lab(4 "More than 3 workers")  pos(5) col(2))   xtitle("Productivity") by(empstat_)
* graph export "${root1}Outputs\Figures\GNB\GNB_kden_productivity_enterprises_size_empstat.png", replace
*------------

*------------
twoway kdensity productivity_b if size_enterprise == 1 || kdensity productivity_b if size_enterprise == 2 || kdensity productivity_b if size_enterprise == 3 || kdensity productivity_b if size_enterprise == 4 , legend(lab (1 "1 worker") lab(2 "2 workers") lab(3 "3 workers") lab(4 "More than 3 workers")  pos(5) col(2))   xtitle("Productivity") by(sector_)
* graph export "${root1}Outputs\Figures\GNB\GNB_kden_productivity_enterprises_size_sector.png", replace
*------------
...
label var rural_2 "Rural"

*tab female , gen(female_)
label var female_2 "Female"
...
recode region (9 = 1) (else = 0), gen(capital)
label var capital "Capital"

preserve
keep productivity_b size_enterprise_2 size_enterprise_3 size_enterprise_4 informal_2 informal2_2 sector2_2 sector2_3 sector2_4 sector2_5 rural_2 female_2 educ_2 educ_3 educ_4 age agesq country1 capital
save "${d_raw}\working\GNB_firm_reg.dta", replace
restore 
