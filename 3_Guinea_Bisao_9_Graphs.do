	...
	* inc_d_salw
	foreach inc in inc_by_hour inc_d_total inc_d_selfw inc_d_salw  {  
		...
		*	
		...
		nopomatch age age_sq  $edulvl	$hh_charac	`add_control'	, outcome(`inc') by(female) fact(wgt) sd filename("${dir_out}/Tables/GNB/OB_N_1_`inc'") replace
		*
		restore
		
		preserve
		cap noi drop _supp _match
		nopomatch age age_sq  $edulvl	$sector		$hh_charac   `add_control'  , outcome(`inc') by(female) fact(wgt) sd filename("${dir_out}/Tables/GNB/OB_N_2_`inc'") replace
		*
		restore
		
		preserve
		cap noi drop _supp _match
		nopomatch age age_sq  $edulvl	$sector		$hh_charac	`add_control'  , outcome(`inc') by(female) fact(wgt) sd filename("${dir_out}/Tables/GNB/OB_N_3_`inc'") replace
		restore
	}
