...
*	Created: May 13, 2024
	...
	use "${d_raw}\EHCVM\SEN\Datain\Menage\s10b_me_SEN2021.dta", clear
...
*	>> Daily income  
	...
	save "${d_raw}\working\SEN_income_enter_noagr.dta" , replace
...
*	>> Hours
	...
	merge m:1 grappe menage using "${d_raw}\working\SEN_income_enter_noagr.dta"  , nogen
	...
	*hist nab_incomepc
	save "${d_raw}\working\SEN_income_enter_noagr_individual.dta", replace



// Section 16. Agriculture -----------------------------------------------------
use "${d_raw}\EHCVM\SEN\Datain\Menage\s16d_me_SEN2021.dta", clear
...
save "${d_raw}\working\SEN_income_agr.dta", replace

// Hours
	use "${d_raw}\EHCVM\SEN\Datain\Menage\s16a_me_SEN2021.dta", clear
	...
	merge m:1 grappe menage using "${d_raw}\working\SEN_income_agr.dta", nogen
	merge m:1 grappe menage using "${d_raw}\working\SEN_ehcvm_aut_SEN2021.dta", nogen
	...
	*hist agb_incomepc  // Ok!
	save "${d_raw}\working\SEN_income_agr_individual.dta", replace


// Section 17. Livestock -------------------------------------------------------
use "${d_raw}\EHCVM\SEN\Datain\Menage\s17_me_SEN2021.dta", clear
...
*	>> Daily income 
	...
	save "${d_raw}\working\SEN_income_livestock.dta", replace

	* ASK ABOUT INDIVIDUAL income here! 
	// Section 18. Fishing ---------------------------------------------------------
	* Three part answer or sources of fishing income. Cost, sales, 
	// a.
	use "${d_raw}\EHCVM\SEN\Datain\Menage\s18_1_me_SEN2021.dta", clear

	collapse (sum) s18q09 s18q10 s18q11 s18q12 s18q13, /// s18q08__1 s18q08__3 s18q08__4 s18q08__5 s18q08__2
	 by(grappe menage )

	save "${d_raw}\working\\SEN_cost_fish.dta", replace

	// b.
	use "${d_raw}\EHCVM\SEN\Datain\Menage\s18_2_me_SEN2021.dta", clear

	collapse (sum) s18q19, by(grappe menage )

	save "${d_raw}\working\SEN_sales1_fish.dta", replace

	// c.
	use "${d_raw}\EHCVM\SEN\Datain\Menage\s18_3_me_SEN2021.dta", clear

	collapse (sum) s18q25, by(grappe menage )

	merge 1:1 grappe menage using "${d_raw}\working\SEN_cost_fish.dta"
	drop _merge

	merge 1:1 grappe menage using "${d_raw}\working\\SEN_sales1_fish.dta"
	drop _merge
	...
	*g income_fishing_d = ((s18q19 + s18q25)/30) - (( s18q10 + s18q11 + s18q12 + s18q13 + s18q08__1 + s18q08__3 + s18q08__4 + s18q08__5 + s18q08__2)/360) 
	...
	save "${d_raw}\working\SEN_income_fish.dta", replace
...
// Hours fishing activities 
	...
	merge m:1 grappe menage using "${d_raw}\working\SEN_income_fish.dta", nogen
	merge m:1 grappe menage using "${d_raw}\working\SEN_ehcvm_aut_SEN2021.dta", nogen
	...
	save "${d_raw}\working\SEN_income_fish_individual.dta", replace
		
	
	
// Section 19.  Agricultural equipment------------------------------------------
	use "${d_raw}\EHCVM\SEN\Datain\Menage\s19_me_SEN2021.dta", clear
	...
	* Collapse the sum of income from rents of agricultural equipment by household
	...
	save "${d_raw}\working\SEN_income_agrequip.dta", replace
	...
	*-------------------------------------------------------------------------------
	use "${d_raw}\working\SEN_individual.dta", clear
	collapse (first) ES nworker (mean) hours age female educy sector_* empstat_* rural , by(grappe menage)
	gen agesq = age^2
	save "${d_raw}\working\SEN_eqhh_size.dta", replace


	use "${d_raw}\EHCVM\SEN\Dataout\ehcvm_welfare_SEN2021.dta",clear

	merge 1:1 grappe menage using "${d_raw}\working\SEN_income_enter_noagr.dta", nogen
	merge 1:1 grappe menage using "${d_raw}\working\SEN_income_agr.dta", nogen
	merge 1:1 grappe menage using "${d_raw}\working\SEN_income_livestock.dta", nogen
	merge 1:1 grappe menage using "${d_raw}\working\SEN_income_fish.dta", nogen
	merge 1:1 grappe menage using "${d_raw}\working\SEN_eqhh_size.dta", nogen

	*Agregar el autoconsumo
	merge 1:1 grappe menage using "${d_raw}\working\SEN_ehcvm_aut_SEN2021.dta", nogen
	...
	save "${d_raw}\working\SEN_self_income.dta", replace
	...
	*-------------------------------------------------------------------------------
	...
	merge 1:1 grappe menage s01q00a using "${d_raw}\working\SEN_income_enter_noagr_individual.dta", nogen
	merge 1:1 grappe menage s01q00a using "${d_raw}\working\SEN_income_agr_individual.dta", nogen
	*merge 1:1 grappe menage s01q00a using "${d_raw}\working\SEN_income_livestock_.dta", nogen  // 
	merge 1:1 grappe menage s01q00a using "${d_raw}\working\SEN_income_fish_individual.dta", nogen
	...
	save "${d_raw}\working\SEN_self_income_individual.dta", replace
	...
	di "${d_raw}\working\"
	di "${d_raw}\EHCVM\SEN\Datain\Menage"
