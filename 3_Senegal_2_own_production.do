...
*/
	...
	** Open the latest version of the consumption file for `cty'
	use "${d_raw}\EHCVM\SEN\Dataout\ehcvm_conso_SEN2021.dta", clear 
	...
	* Keep only autoconsumption
	...
	/* SEN has codes 140 to 146 and GIN has code 165, but no label in the codpr variable, so I decided to 
	...
	**////////////////////////////////////////////////////////////////
	use "${d_raw}\EHCVM\SEN\Datain\Menage\s16b_me_SEN2021.dta" , clear 
...
/* 
	...
	* Merge with the welfare file to use the geographic variables 
	merge m:1 grappe menage using "${d_raw}\EHCVM\SEN\Dataout\ehcvm_welfare_SEN2021.dta", nogen keep(match) keepusing(vague zae milieu)
	...
	* Generate variable of annual autoconsumption in chemical fertilizer
	...
	save  "${d_raw}\working\SEN_Aut_S16b.dta", replace // Save autoconsumption aggregates of templates from section 16
	...
	* Merge all data sets with autoconsumption variables that need treatment of outliers
	merge 1:1 grappe menage using "${d_raw}\working\SEN_Aut_S16b.dta", nogen
	...
	* Save autoconsumption file before calculating totals for each 4-digit code
	save "${d_raw}\working\SEN_ehcvm_aut_desag_SEN2021.dta" , replace 
	...
	* Save autoconsumption file
	save "${d_raw}\working\SEN_ehcvm_aut_SEN2021.dta", replace
