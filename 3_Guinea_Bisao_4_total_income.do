...
*-------------------------------------------------------------------------------
	...
	*-------------------------------------------------------------------------------
	...
	merge 1:m grappe menage using "${d_raw}\working\GNB_individual.dta"
	drop if _merge == 2
	drop _merge

	merge m:1 grappe menage using "${d_raw}\working\GNB_self_income.dta", nogen
	merge 1:1 grappe menage s01q00a using "${d_raw}\working\GNB_self_income_individual.dta" // New version
	drop if _merge == 2
	drop _merge

	// salary workers & non-salary
	merge 1:1 grappe menage s01q00a using "${d_raw}\EHCVM\GNB\Datain\Menage\s04b_me_GNB2021.dta"
	ren _merge _merge_a

	merge 1:1 grappe menage s01q00a using "${d_raw}\EHCVM\GNB\Datain\Menage\s04c_me_GNB2021.dta",nogen

	// sociodemographic 
	merge m:1 grappe menage s01q00a using  "${d_raw}\EHCVM\GNB\Datain\Menage\s01_me_GNB2021.dta"
	ren _merge _merge_b
...
save "${d_raw}\working\GNB_individual_2.dta", replace
...
use "${d_raw}\working\GNB_individual_2.dta", clear
gen y_imp=. 
gen y_res=1
...
tempfile y_imputes 
save `y_imputes'
save "${d_raw}\working\GNB_imputes" , replace 
restore
...
//  a. indvidual data
* use "${d_raw}\working\GNB_individual_2.dta", clear
use "${d_raw}\working\GNB_individualall_2.dta", clear
merge 1:1 grappe menage s01q00a using "${d_raw}\working\GNB_imputes" , gen(merge_labforce) // `y_imputes'
gen lnwage_h=ln(wage_h+1)
...
g country = "GNB"
save "${d_raw}\working\GNB_individual_reg.dta", replace
restore
...
save "${d_raw}\working\GNB_individual_reg_FULL.dta", replace
